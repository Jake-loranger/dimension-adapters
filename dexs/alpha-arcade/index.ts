...
const fetch = async (options: FetchOptions) => {
  ...
  do {
    ...
    // Append each batch of alphaArcadeTxns to a file as the loop runs
    // JSONL (JSON Lines) format stores one JSON object per line, making it easy to append and process large datasets line by line.
    const path = './alphaArcadeTxns.jsonl';
    for (const txn of alphaArcadeTxns) {
      fs.appendFileSync(path, JSON.stringify(txn) + '\n');
    }
    for (const txn of alphaArcadeTxns) {
      ...
      } else if (hasAnyTargetAppArg(txn, ['uh3u9Q=='])){
        console.warn("Processing MATCH transaction with id:", txn.id);
        if (Array.isArray(txn['inner-txns'])) {
          for (const inner of txn['inner-txns']) {
            const assetTransfer = inner['asset-transfer-transaction'];
            if (assetTransfer && assetTransfer.amount) {
              const assetName = assetTransfer["asset-id"] === 31566704 ? "USDC" : assetTransfer["asset-id"];
              console.log("MATCH (inner):", assetTransfer.amount / 1e6, "with asset:", assetName, "with txnId:", txn.id);
              dailyVolume += assetTransfer.amount;
            }
          }
        }
